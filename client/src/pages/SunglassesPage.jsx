...
export default function SunglassesPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ category: 'sunglasses', limit: 24 });
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-main)' }}>
      ...
      <div className="relative pt-32 pb-16" style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border-soft)' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <p className="label-luxury mb-3">Eyewear Collection</p>
            <h1 className="heading-display" style={{ fontSize: 'clamp(36px, 6vw, 64px)' }}>Sunglasses</h1>
            <p className="text-sm mt-3 max-w-lg" style={{ color: 'var(--text-muted)' }}>
              Discover frames that define the moment — curated by glass type, not by brand.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        ...
        {/* Glass type strip — SUNGLASSES ONLY, NO BRANDS */}
        {categories.length > 0 && (
          <div className="mb-12">
            <p className="label-luxury mb-4">Filter by Glass Type</p>
            <div className="flex gap-2 overflow-x-auto pb-2">
              <button
                onClick={() => setFilters(f => ({ ...f, glassCategory: '' }))}
                className="px-5 py-2.5 text-xs uppercase tracking-widest whitespace-nowrap transition-colors"
                style={!filters.glassCategory
                  ? { background: 'var(--accent-gold)', color: '#080808' }
                  : { border: '1px solid var(--border-soft)', color: 'var(--text-muted)' }}
              >All Types</button>
              {categories.map(c => (
                <button key={c._id}
                  onClick={() => setFilters(f => ({ ...f, glassCategory: c._id }))}
                  className="px-5 py-2.5 text-xs uppercase tracking-widest whitespace-nowrap transition-colors"
                  style={filters.glassCategory === c._id
                    ? { background: 'var(--accent-gold)', color: '#080808' }
                    : { border: '1px solid var(--border-soft)', color: 'var(--text-muted)' }}
                >{c.name}</button>
              ))}
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p style={{ color: 'var(--text-muted)' }}>No sunglasses found for these filters.</p>
          </div>
        ) : (
